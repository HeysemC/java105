...
public class C03_SifreKontrolu {
    public static void main(String[] args) {
        ...
        do {
            ...
            // flag 4 ise sifre tamamdır, degilse hata vardir
        }while (flag!=4);
        {
            System.out.println("sifreniz basasri ile kaydedilmistir");
        }
    }
    public static int ozelKarakterKontroluYap(String sifre) {
       int flag=0;
       String ozelKarakterler="+-*/_-?*§±@!$%^&*()_-+=?/>.<,`~";
       //sifre ozel karakter icermelidir
        ...
        //bizin icin yeterli olur
        for (int i = 0; i < sifre.length(); i++) {
            if (ozelKarakterler.contains(sifre.substring(i, i + 1))) {
                flag++;
                break;
            }
            } //bayra==0 ise kucuk harf yoktur ve hata vardir bunu yazdiralim
            if (flag == 0) {
    ...
    public static int buyukHarfKontroluYap(String sifre) {
        ...
        // Sifre kucuk harf icermelidir
        //bunun icin tum harfleri kontrol edecek bir for loop olusturmaliyim
        //1 tane kucuk harf bulmam yeterli
        for (int i = 0; i < sifre.length(); i++) { // sifre.length() ; -1 otomatik geldi
